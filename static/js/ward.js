...
(function() {
    ...
    $('#xslxUpload').change(() => {
        ...
        reader.onload = (event) => {
            ...
            showTab1Table(tableColTitle, xlsxDataArray);
        };
        // 开始读取文件
        if (rABS) {
    ...
    function showTab1Table(title, data) {
        if (table) {
            table.clear();
            // xlsxDataArray.forEach(v => table.row.add(v));
    ...
    function showTab2Table(data) {
        ...
        if (table2) {
            table2.clear();
            // xlsxDataArray.forEach(v => table2.row.add(v));
            table2.rows.add(dataInRows).draw();
