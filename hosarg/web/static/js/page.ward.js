...
define([
    ...
    'datepicker',
    'datepicker.zh-CN'
], function($, XLSX, common, DataTableModule, Utils, CRC, NavTab, DataCheck) {
    'use strict';
    let table = null;
    let table2 = null;
    let table3 = null;
    ...
    function page_init() {
        ...
        const tabCheckers = {
            '1': [{
                ...
                {
                    dom: '#xslxUploadBtn',
                    checker: () => xlsxDataArray.length ? null : '请选择文件'
                }
            ],
            '2': [],
            '3': [{
                ...
                },
                {
                    ...
                    checker: (val) => {
                        ...
                        let startDate = parseInt($('#startday').val());
                        let endDate = parseInt(val);
                        if (endDate < startDate) {
                            return '请确保结束日期晚于开始日期';
                        }
                        return null;
                    }
                }
        ...
        function errCheckCallback(t) {
            return new DataCheck().formDataChecker(tabCheckers[t]);
        }

        new NavTab('#mytab', '#nextBtn', '#backBtn', '#finishBtn',
            navTabCallBack, finishBtnCallback, errCheckCallback
        );
    ...
    function navTabCallBack(lstTab, tarTab) {
        if (lstTab === 2) {
            BackupSenddata();
            BackupClearInterval();
        }
        ...
        if (tarTab === 1) {
            ...
            if (lstTab === 1) {
                if (!curMonth)
                    curMonth = $('#fileMonth').val().split('/').join('');
                ...
                }, () => common.showNotification('历史数据保存成功！', 'success'), () => common.showNotification('历史数据保存失败！', 'danger'));
            }
            ...
            // 初始化日期选择器
            $('[data-toggle="daypicker"]').datepicker({
                format: 'dd',
                language: 'zh-CN',
                autoHide: true,
                date: $('#fileMonth').datepicker('getDate')
            });
    ...
    $('#xslxUpload').change(() => {
        ...
        reader.onload = (event) => {
            ...
            tableColTitle = [];
            xlsxTitleArray.forEach(v => tableColTitle.push({
                title: v
            }));
            // 获取DataTableModule类实例
